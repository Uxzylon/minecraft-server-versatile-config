...
Add tracked entries:
  ...
  add papermc <project> [dest] [--version latest_any|latest_stable|latest_snapshot|selector]
  add modrinth <project> [dest] [--version latest_any|latest_stable|latest_beta|selector]
  add curseforge <project-id> [dest] [--version latest_any|latest_stable|latest_beta|selector]
...
Examples:
  ...
  ./mc-updater.sh add papermc velocity velocity --version latest_snapshot
  ./mc-updater.sh add modrinth fabric-api mods --version latest_stable
  ./mc-updater.sh add modrinth limboapi velocity/plugins --ignore-game-version
...
install_resolved_entry() {
  ...
  update_env_var_file "$env_key" "$filename"
  update_env_filename_references "$old_filename" "$filename"
...
resolve_fabric_server() {
  ...
  [[ -n "$installer" && "$installer" != "null" ]] || { err "could not resolve Fabric installer"; return 1; }
  [[ -n "$loader" && "$loader" != "null" ]] || { err "could not resolve Fabric loader"; return 1; }
...
resolve_modrinth() {
  ...
  [[ -n "$file" && "$file" != "null" ]] || { err "no Modrinth download file found for $project_ref"; return 1; }
...
curseforge_get() {
  ...
  [[ -n "$CURSEFORGE_API_KEY" ]] || { err "CURSEFORGE_API_KEY is required"; return 1; }
...
resolve_curseforge() {
  ...
  [[ "$project_id" =~ ^[0-9]+$ ]] || { err "CurseForge expects a numeric project id"; return 1; }
  ...
  [[ -n "$url" && "$url" != "null" ]] || { err "CurseForge returned no download URL for $project_id/$file_id"; return 1; }
...
resolve_url() {
  ...
  [[ -n "$filename" ]] || { err "could not infer filename for URL entry $id; pass --filename"; return 1; }
...
parse_flags() {
  ...
  while (($# > 0)); do
    case "$1" in
      --dest) shift; DEST="${1:-}" ;;
      --loader) shift; LOADER="${1:-}"; LOADER_EXPLICIT="true" ;;
      --mc|--game-version) shift; GAME_VERSION="${1:-}" ;;
      --force|--ignore-game-version) IGNORE_GAME_VERSION="true" ;;
      --ignore-loader) IGNORE_LOADER="true" ;;
      --ignore-compatibility) IGNORE_GAME_VERSION="true"; IGNORE_LOADER="true" ;;
      --env-key) shift; ENV_KEY="${1:-}" ;;
      --version) shift; VERSION="$(normalize_requested_version "${1:-$MC_UPDATER_DEFAULT_VERSION}")" ;;
...
add_entry() {
  ...
  [[ -n "$type" ]] || { err "usage: ./mc-updater.sh add <type> ..."; exit 2; }
  parse_flags "$@"
  ...
  case "$type" in
    fabric-server|fabric)
      ...
      install_resolved_entry "$resolved" "$dest" false "$LOADER" "$GAME_VERSION" "$ENV_KEY" "$VERSION" "$IGNORE_GAME_VERSION" "$IGNORE_LOADER"
      ;;
    papermc|paper|velocity)
      project="${POSITIONALS[0]:-}"
      [[ "$type" == "velocity" && -z "$project" ]] && project="velocity"
      dest="${POSITIONALS[1]:-.}"
      [[ -n "$project" ]] || { err "usage: ./mc-updater.sh add papermc <project> [dest]"; exit 2; }
    ...
    modrinth|mr)
      ...
      [[ -n "$project" ]] || { err "usage: ./mc-updater.sh add modrinth <project> [dest]"; exit 2; }
    ...
    curseforge|cf)
      project="${POSITIONALS[0]:-}"
      dest="${POSITIONALS[1]:-$DEST}"
      [[ -n "$project" ]] || { err "usage: ./mc-updater.sh add curseforge <project-id> [dest]"; exit 2; }
      LOADER="$(infer_loader_for_dest "$dest")"
    ...
    url)
      ...
      [[ -n "$id" && -n "$url" ]] || { err "usage: ./mc-updater.sh add url <id> <url> [dest]"; exit 2; }
...
resolve_for_existing_entry() {
  ...
  case "$source" in
    ...
    *) err "unknown tracked source: $source"; return 1 ;;
  esac
}

update_one_entry() {
  local entry="$1"
  local resolved old_version new_version name dest disabled env_key loader game_version requested_version ignore_game_version ignore_loader
  ...
  log "updating $name: $old_version -> $new_version"
  install_resolved_entry "$resolved" "$dest" "$disabled" "$loader" "$game_version" "$env_key" "$requested_version" "$ignore_game_version" "$ignore_loader"
}
...
update_entries() {
  ...
  if [[ -n "$selector" ]]; then
    ...
    [[ -n "$entry" ]] || { err "no tracked entry matched: $selector"; exit 1; }
...
remove_entry() {
  ...
  [[ -n "$selector" ]] || { err "usage: ./mc-updater.sh remove <selector>"; exit 2; }
  entry="$(find_entry_json "$selector")"
  [[ -n "$entry" ]] || { err "no tracked entry matched: $selector"; exit 1; }
...
set_enabled_state() {
  ...
  [[ -n "$selector" ]] || { err "usage: ./mc-updater.sh enable|disable <selector>"; exit 2; }
  entry="$(find_entry_json "$selector")"
  [[ -n "$entry" ]] || { err "no tracked entry matched: $selector"; exit 1; }
...
edit_entry() {
  ...
  entry="$(find_entry_json "$selector")"
  [[ -n "$entry" ]] || { err "no tracked entry matched: $selector"; return 1; }
