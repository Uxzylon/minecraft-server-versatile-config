...
SERVER_WORKING_DIR="${SERVER_WORKING_DIR:-$SERVER_WORKING_DIR}"
...
# Runtime files.
RUNTIME_DIR="${RUNTIME_DIR:-$SERVER_WORKING_DIR/.runtime}"
...
#
# working_dir:
#   - Absolute path, or relative to SERVER_WORKING_DIR.
# jar_path:
...
# Set SERVICES_CONFIG to an empty value to run without service jars.
...
start_server() {
	...
	split_words "$SERVER_JAVA_OPTS"
	java_opts=("${SPLIT_RESULT[@]}")
	split_words "$SERVER_ARGS"
	app_args=("${SPLIT_RESULT[@]}")
...
Service env format:
  ...
  Empty SERVICES_CONFIG means no service jars are configured.
