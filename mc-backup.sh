...
SERVER_WORKING_DIR="${SERVER_WORKING_DIR:-$SCRIPT_DIR}"
...
# Comma-separated world directories, relative to SERVER_WORKING_DIR unless absolute.
...
# Patterns are passed to zip/tar from inside SERVER_WORKING_DIR.
...
#   tmux         send keys directly to TMUX_SESSION
#   none         do nothing
...
MC_BACKUP_MESSAGE_FAILED="${MC_BACKUP_MESSAGE_FAILED:-Backup failed. Auto-save is enabled again.}"
...
Minecraft backup helper:
  ...
  restore <archive>            Restore a zip or tar.gz backup into SERVER_WORKING_DIR
  help                         Show this help
...
Environment / .env variables:
  SERVER_WORKING_DIR=.                           Directory to back up
...
send_with_tmux() {
	...
	run_best_effort tmux send-keys -t "$TMUX_SESSION" "$minecraft_command" ENTER
}
...
build_archive_path() {
	...
	local format="$2"
	local timestamp="$3"
	local backup_dir suffix extension

	backup_dir="$(resolve_from_server_dir "$MC_BACKUP_DIR")"
	...
	*) suffix="$scope" ;;
	...
	printf '%s/%s_%s[%s].%s\n' "$backup_dir" "$timestamp" "$MC_BACKUP_NAME" "$suffix" "$extension"
}
...
create_archive() {
	...
	*)
	...
	log "  server dir: $SERVER_WORKING_DIR"
	log "  archive: $archive"

	if ! (
		cd "$SERVER_WORKING_DIR" || exit 1
...
run_backup() {
	local scope="$1"
	local stop_server="$2"
	local started_at ended_at elapsed backup_status
	...
	if bool_is_true "$stop_server"; then
		stop_for_backup
		create_archive "$scope" || backup_status=$?

		if bool_is_true "$MC_BACKUP_RESTART_AFTER_STOP"; then
			start_after_backup
		fi
	else
		warn_and_prepare_hot_backup
	...
	if ((backup_status != 0)); then
		err "backup failed after ${elapsed}s"
...
restore_backup() {
	...
	if bool_is_true "$MC_BACKUP_CONFIRM_RESTORE"; then
		confirm_or_exit "Restore '$archive' into '$SERVER_WORKING_DIR'? This will replace matching files/directories."
	fi
	...
	log "restoring files into: $SERVER_WORKING_DIR"
	local item base
	shopt -s dotglob nullglob
	for item in "$temp_dir"/*; do
		base="$(basename "$item")"
		rm -rf "$SERVER_WORKING_DIR/$base"
		mv "$item" "$SERVER_WORKING_DIR/"
	done
